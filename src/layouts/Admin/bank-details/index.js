...
function AdminBankDetails() {
  ...
  const [selectedFile, setSelectedFile] = useState()
    ...
    const [previewQRCode, setPreviewCode ] = useState('')
    const [newUPI, setNewUpi ] = useState('')
    
    const initialValues = {
        qrCode:  '',
        upiId: upiData[0]?.upi_link || ''
    }
    ...
    const getAll = () => {
      ApiService.getUpiDetails().then((result) => {
    ...
    const handleFileDownload = async (url) => {
      try {
        ...
        const file = new File([response.data], 'qr-code.jpg', { type: 'image/jpeg' || 'image/jpg' || 'image/webp' });
    ...
    {isEditable ? (
        ...
        {upiData && upiData.length > 0 ? (
            <SoftBox
              ...
              variant="gradient"
            >
              <SoftBox component="img" src={upiData && upiData[0]?.qr_image  ? upiData[0]?.qr_image : null} alt="Qr Code"  borderRadius="lg" className="qeCode-height"  />
              <strong className="text-white text-center g-padding-10px">{upiData && upiData[0]?.upi_link || ''}</strong>
            </SoftBox>
            ) : <div className="text-center p-3">No Record Found</div> }
          </Grid>
          <Grid item xs={12}  sx={{ position: "relative", ml: "auto" }}>
         
        {upiData && upiData.length === 0 ?   <SoftButton
            component="span"
            variant="gradient"
            color='info'
            onClick={()=> {
      ...
      validationSchema={upiData && upiData === 0 ? schema : editSchema}
      onSubmit={(values) => {
        setDisabled(true);
        var req = {
          ...
          if(req.qr_image === ""){
            console.log(req.qr_image)
            req.qr_image = previewQRCode
          }


          if(upiData && upiData.length === 0) {
            ApiService.addUPIDetails(req).then((result)=> {
              setDisabled(false);
              getAll()
            }).catch((err)=> {
              setDisabled(false);
            ...
            ApiService.updateUpiDetails(req).then((result)=> {
              setDisabled(false);
              getAll()
            }).catch((err)=> {
              setDisabled(false);
